...
<div style="width: 80%; margin: 15px auto 0;">  
    ...
    <form action="index.php?controlador=Admoncitas&accion=crearcita"  method="POST" onsubmit="return verificar($(this).attr('id'))" id="formnuevacita">                   
        <table style="font-size: 12px" align="center" border="0" width="100%" >       
            <tr><td style="padding: 5px" width="50%" bgcolor="#E4E4E4">Cedula Paciente:</td>
                <td style="padding: 5px" width="50%" bgcolor="#F2F2F2"> <input name="cedula" id="cedula" value="<?php echo $cedula ?>" /> </td>
            </tr>
...
<script>
    ...
    function verificar(id){        
        ...
        if($('#hora > option').length == 0 ||($('#cedula').val()).length==0 ){            
            ...
            $.ajax({
                ...
                {    
                    if(responseText.respuesta=='si'){
                        ...
                        rta=false;
                        window.location="index.php?controlador=Admoncitas&accion=registrarUser&cedula="+$('#cedula').val();
            ...
            retorno = true;
            if(rta){
                var values={hora: $('#hora').val(),dia: $('#dia').val()};                
                $.ajax({
                    type: "POST",               
                    dataType: "json",              
